...
void update_u_offload_dual_border(double ***f0, double ***f1, double ***u0, double ***u1, double ***u_old0, double ***u_old1, int N);
...
void jacobi_offload_dual_border(double ***f0, double ***f1, double ***u0, double ***u1, double ***u_old0, double ***u_old1, int N, int iter_max) {
    for(int iter=0; iter < iter_max; iter++) {
        swap_pointers((void **)&u0, (void **)&u_old0);
        swap_pointers((void **)&u1, (void **)&u_old1);
        update_u_offload_dual_border(f0, f1, u0, u1, u_old0, u_old1, N);
    }
}
...
void update_u_offload_dual_border(double ***f0, double ***f1, double ***u0, double ***u1, double ***u_old0, double ***u_old1, int N) {
    ...
    for (int j = 1; j < N-1; j++) {
        for (int k = 1; k < N-1; k++) {
            u0[i0][j][k] = frac * (u_old0[i0-1][j][k] + u_old1[i1][j][k] 
                + u_old0[i0][j-1][k] + u_old0[i0][j+1][k] + u_old0[i0][j][k-1] 
                + u_old0[i0][j][k+1] + delta2*f0[i0][j][k]);
        }
    }
    ...
    for (int j = 1; j < N-1; j++) {
        for (int k = 1; k < N-1; k++) {
            u1[i1][j][k] = frac * (u_old0[i0][j][k] + u_old1[i1+1][j][k] 
                + u_old1[i1][j-1][k] + u_old1[i1][j+1][k] + u_old1[i1][j][k-1] 
                + u_old1[i1][j][k+1] + delta2*f1[i1][j][k]);
        }
    }
